...
export const InboxItemComponent = (props: InboxItemProps) => {
  ...
  return (
    <Animated.View style={tailwind.style(
      `mx-3 mb-2 rounded-2xl overflow-hidden py-3 ${isUnread ? 'bg-blue-50 border-l-[3px] border-l-blue-700 pl-[9px] pr-3' : 'bg-white px-3 border border-gray-200'}`
    )}>
      <AnimatedNativeView
        ...
        <AnimatedNativeView
          ...
          <Animated.Text
            ...
            style={tailwind.style(
              `text-lg ${isUnread ? 'font-inter-semibold-20' : 'font-inter-medium-24'} tracking-[0.24px] text-gray-950 capitalize`,
              `max-w-[${width - 250}px]`,
            )}>
            {sender.name || ''}
          </Animated.Text>
          <NativeView style={tailwind.style('flex flex-row items-center gap-0.5')}>
            <Animated.Text style={tailwind.style('text-sm font-inter-420-20 text-gray-400')}>
              #
            </Animated.Text>
            <Animated.Text style={tailwind.style('text-sm font-inter-420-20 text-gray-400')}>
              {conversationId}
        ...
        <AnimatedNativeView style={tailwind.style('flex flex-row items-center gap-2')}>
          ...
          <NativeView>
            <Animated.Text
              style={tailwind.style(
                'text-sm font-inter-420-20 leading-[16px] tracking-[0.32px] text-gray-500',
              )}>
              {lastActivityAt()}
            </Animated.Text>
          </NativeView>
      ...
      <Animated.View style={tailwind.style('flex flex-row justify-between mt-1.5')}>
        <Animated.View style={tailwind.style('flex flex-row items-center gap-1.5 flex-1')}>
          ...
          <Animated.Text
            style={tailwind.style(
              `text-md ${isUnread ? 'font-inter-medium-24 text-gray-800' : 'font-inter-420-20 text-gray-500'} leading-[17px] tracking-[0.32px] flex-shrink`,
            )}
            numberOfLines={1}
            ellipsizeMode="tail">
            {pushMessageTitle}
